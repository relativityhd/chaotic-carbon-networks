...
def assert_dims(x: xr.DataArray):
    if len(x.dims) == 2:
        ...
        assert axis_is_hex(x, "vertex"), "if x has 2 dimensions the vertex dimension must have a hex_res attribute"
    else:
        assert "time" in x.dims, "x must have time dimension"
...
def xrmatrix_from_func(x: xr.DataArray, y: xr.DataArray, f: Callable[[np.ndarray, np.ndarray], np.ndarray]):
    ...
    if x_is_hex:
        m.coords["vertex"].attrs["hex_res"] = x.coords["vertex"].attrs["hex_res"]
    if y_is_hex:
        m.coords["vertex_other"].attrs["hex_res"] = y.coords["vertex"].attrs["hex_res"]
    return m
...
def link_lengths_like(m: xr.DataArray):
    ...
    x_hex = axis_is_hex(m, "vertex")
    y_hex = axis_is_hex(m, "vertex_other")
